...
class NARStageDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        hidden_states = self.self_attn_layer_norm(hidden_states, nar_stage)
        ...
        if encoder_hidden_states is not None:
            ...
            hidden_states = nn.functional.dropout(hidden_states, p=self.dropout, training=self.training)
            hidden_states = residual + hidden_states
            hidden_states = self.encoder_attn_layer_norm(hidden_states, nar_stage)
        ...
        hidden_states = nn.functional.dropout(hidden_states, p=self.dropout, training=self.training)
        hidden_states = residual + hidden_states
        hidden_states = self.final_layer_norm(hidden_states, nar_stage)
